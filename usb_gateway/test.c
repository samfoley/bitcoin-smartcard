...
Point* ec_point_mul(Point *P, BIGNUM *n, Curve *c)
{
	...
	else
	{
		if(BN_is_odd(n))
		{
			...
			// lambda = (3x^2 + b)/2y
			BN_mod_sqr(lambda, P->x, mod, ctx);			
			BN_mul_word(lambda, 3);			
			BN_mod_add(tmp, lambda, c->b, mod, ctx);
			BN_rshift1(tmp,tmp);
			BN_div(lambda, NULL, tmp, P->y, ctx);
			
			// xr = lambda^2 - 2x
			BN_mod_sqr(tmp, lambda, mod, ctx);
			BN_lshift1(tmp2, P->x);
			BN_mod_sub(xr, tmp, tmp2, mod, ctx);
			
			// yr = lambda(x - xr) - y
			BN_mod_sub(tmp, P->x, xr, mod, ctx);
			BN_mod_mul(tmp2, lambda, tmp, mod, ctx);
			BN_mod_sub(yr, tmp2, P->y, mod, ctx);
			...
			// P + f(P, n-1)
			BN_mod_sub(tmp, n, BN_value_one(), mod, ctx);
			p2 = ec_point_mul(P, tmp, c);
			
			// lambda = (yq-yp)/(xq-xp)
			BN_mod_sub(tmp, p2->y, P->y, mod, ctx);
			BN_mod_sub(tmp2, p2->x, P->x, mod, ctx);
			BN_div(lambda, NULL, tmp, tmp2, ctx);
			
			// xr = lambda^2 - xp - xq
			BN_mod_sqr(tmp, lambda, mod, ctx);
			BN_mod_sub(tmp2, tmp, P->x, mod, ctx);
			BN_mod_sub(xr, tmp2, p2->x, mod, ctx);
